...
static void process_camera_image() {
  const unsigned char *image = wb_camera_get_image(camera);
  int width = wb_camera_get_width(camera);
  int height = wb_camera_get_height(camera);

  // Example: get brightness of center pixel
  int r = wb_camera_image_get_red(image, width, width / 2, height / 2);
  int g = wb_camera_image_get_green(image, width, width / 2, height / 2);
  int b = wb_camera_image_get_blue(image, width, width / 2, height / 2);
